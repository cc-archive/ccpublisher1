...
class Mp3Metadata(AudioMetadata):
	...
	if filename is not None:
            if not(isinstance(filename, unicode)):
                self.filename = unicode(filename, sys.getfilesystemencoding())
    ...
    def setClaim(self, claim):
	    ...
	    print "getting ready to call open"
	    self.__open()
	    print self.__tag
    ...
    def isWritable(self):
	"""Returns true if the user has permission to change the metadata."""
        # XXX Ugly hack to work around os.access barfing on unicode on Py 2.3
