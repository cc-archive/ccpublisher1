...
class FileFormatWizPage(xrcwiz.XrcWizPage):
    # default extension formats
    FORMAT_EXTS = {'wav': 'WAVE',
                   'ogg': 'Ogg Vorbis',
                   'flac': 'Flac',
                   'divx': 'DivX',
                   'jpg' : 'JPEG',
    ...
    def __detachAll(self):
	    ...
	    self.files[filename][0].Destroy()
	    self.files[filename][1].Destroy()
    ...
    def setFiles(self, files):
        ...
        for fn in self.filenames:
            ...
            # add the combo box
            work_format = XRCCTRL(self.parent.getPage('WORK_METADATA'), 
                                  'CMB_WORK_FORMAT').GetValue().lower()
            if work_format in pyarchive.const.FILE_FORMATS.keys():
               choices = pyarchive.const.FILE_FORMATS[work_format]
            else:
               choices = pyarchive.const.VALID_FORMATS

	    if (fn in self.formats) and (self.formats[fn] is not None):
	       default = self.formats[fn]
	    else:
	       default = ''

            cmb = wx.ComboBox(self.panel,
                         style=wx.CB_DROPDOWN|wx.CB_READONLY,
                         choices=choices, value=default)

            cmb._filename = fn
            self.panel.GetSizer().Add(cmb, flag=wx.EXPAND)

            self.files[fn] = (label, cmb)
	    if fn not in self.formats.keys():
        ...
        try:
            ...
            # catch errors that arise because the user hasn't
            # selected the work format yet
            pass
        
        self.Layout()
	self.panel.Layout()

    def setFiles_old(self, files):
        self.__detachAll()
        ...
        for fn in self.filenames:
            ...
            # try to determine the default file format
            exten = fn.split('.')[-1]
            if exten in self.FORMAT_EXTS:
                def_format = self.FORMAT_EXTS[exten]
            else:
                def_format = ''
            ...
            cmb = wx.ComboBox(self.panel,
                         style=wx.CB_DROPDOWN|wx.CB_READONLY,
                         choices = choices, #pyarchive.const.VALID_FORMATS,
                         value = def_format
                         )
            cmb._filename = fn
            self.panel.GetSizer().Add(cmb, flag=wx.EXPAND)
